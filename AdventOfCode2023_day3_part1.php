...
.664.598..
TESTINPUT;
    public function testAdventPart1()
    {
        $numbers=[];
        $byLine = explode("\n", $this->input);
        ...
        for ($i = 0; $i < count($byLine); $i++) {
            for ($j = 0; $j < count($byLine[$i]); $j++) {
                $value = $byLine[$i][$j];
                $isSymbol = preg_match('/[^a-zA-Z0-9\s\.]/', $value);
                if ($isSymbol) {
                    $matchingNumbers = $this->findMatchingNumbersPart1($numbers, $j, $i);
                    $sum+=array_sum($matchingNumbers);
                }
            }
        }
        var_dump($sum);
     ...
     * @param $y
     */
    private function findMatchingNumbersPart1($numbers, $x, $y)
    {
        $matchingNumbers = [];
