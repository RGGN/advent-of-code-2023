...
class AdventOfCode2023 extends TestCase
{
private $input = <<<INPUT
...
TESTINPUT;
    ...
    public function testAdventPart2()
    {
        $numbers=[];
        $byLine = explode("\n", $this->input);
        foreach ($byLine as $linePosition => $line) {
            $byLine[$linePosition] = str_split($line);
            preg_match_all('/\d+/', $line, $matches, PREG_OFFSET_CAPTURE);
            foreach($matches[0] as $match)
            {
                $numbers[] = new Number($match[1], $linePosition, $match[0]);
            }
        }

        $sum = 0;
        for ($i = 0; $i < count($byLine); $i++) {
            for ($j = 0; $j < count($byLine[$i]); $j++) {
                $value = $byLine[$i][$j];
                $isSymbol = $value === "*";
                if ($isSymbol) {
                    $matchingNumbers = $this->findMatchingNumbersPart2($numbers, $j, $i);
                    if($matchingNumbers)
                    {
                        $sum+=array_product($matchingNumbers);
                    }
                }
            }
        }
     ...
     * @param $y
     */
    private function findMatchingNumbersPart2($numbers, $x, $y)
    {
        $matchingNumbers = [];
        foreach ($numbers as $number) {
            if (($number->matchesY($y - 1) && $number->matchesX($x - 1)) ||
                ($number->matchesY($y - 1) && $number->matchesX($x)) ||
                ($number->matchesY($y - 1) && $number->matchesX($x + 1)) ||
                ($number->matchesY($y) && $number->matchesX($x - 1)) ||
                ($number->matchesY($y) && $number->matchesX($x + 1)) ||
                ($number->matchesY($y + 1) && $number->matchesX($x - 1)) ||
                ($number->matchesY($y + 1) && $number->matchesX($x)) ||
                ($number->matchesY($y + 1) && $number->matchesX($x + 1))
            ) {
                $matchingNumbers[] = (int)$number->number;
            }
        }

        if(count($matchingNumbers) == 2) return $matchingNumbers;
        return [];
    }
...
class Number{
    ...
    public function __construct($posXinit, $posY, $number)
    {
        $this->PosXs = range($posXinit, $posXinit+strlen($number)-1);
        $this->posY = $posY;
        $this->number = $number;
    }

    public function matchesX($x)
    {
        return in_array($x, $this->PosXs);
    }
    public function matchesY($y)
    {
        return $y == $this->posY;
    }
}
